...
@Repository
public class JdbcTaskRepositoryImpl implements TaskRepository, RowMapper<Task> {
    private final JdbcOperations jdbcOperations;

    public JdbcTaskRepositoryImpl(JdbcOperations jdbcOperations) {
    ...
    public void save(Task task) {
        this.jdbcOperations.update("""
                ...
                values (?, ?, ?, ?)
                """, new Object[]{task.id(), task.details(),
                task.completed(), task.applicationUserId()});
    }

    @Override
    public Optional<Task> findById(UUID id) {
        return this.jdbcOperations.query("""
                        select * from t_task where id=?
                        """, new Object[]{id}, this)
                .stream().findFirst();
    }
    ...
    public List<Task> findByApplicationUserId(UUID id) {
        return this.jdbcOperations.query("""
                select * from t_task where id_application_user=?
                """, this, id);
    }
